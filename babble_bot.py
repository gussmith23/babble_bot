...
@bot.message_handler(func=lambda m: int(m.date) > since and "/info" not in m.text and (random.random() < freq or (m.text is not None and "@babble_bot" in m.text)))
def mangle_message(message):
	...
	if message_info['all_messages']	is False:
		bot.send_message(message.chat.id, 
											"_{}_".format(m.mangle("One or more languages not recognized.", 3)),
											parse_mode = "Markdown")
		return
...
@bot.message_handler(commands=['info'], func=lambda m: m.reply_to_message is not None and m.reply_to_message.message_id in messages and int(m.date) > since)
...
def parse(text):
	...
	if language_list_result:
		language_list_string = language_list_result.group(0)[1:-1] # remove brackets on the sides
